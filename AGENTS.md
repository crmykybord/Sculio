...
- **Contratos del runtime verificados contra SMODS 1814a** (dump en `lovely/dump/`):
  ...
  - El uso de consumibles se trackea con el contexto oficial `using_consumeable` (disparado por `G.FUNCS.use_card` tras `use_consumeable`), recibido en `Sculio:calculate`; NO hookear `Card:use_consumeable` manualmente.
